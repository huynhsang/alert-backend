package com.vn.sanght.challenge.common.util;
...
public final class ResponseUtil {
    ...
    public static <X> ResponseEntity<X> wrapOrNotFound(
            final Optional<X> maybeResponse) {
        return wrapOrNotFound(maybeResponse, (HttpHeaders) null);
    }

    public static <X> ResponseEntity<X> wrapOrNotFound(
            final Optional<X> maybeResponse, HttpHeaders header) {
        return (ResponseEntity<X>) maybeResponse.map((response) -> {
            return ((BodyBuilder) ResponseEntity.ok().headers(header))
                    .body(response);
        }).orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }

    public static <X> ResponseEntity<X> createdOrNot(
            final Optional<X> maybeResponse) {
